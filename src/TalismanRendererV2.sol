...
/// @title TalismanRendererV2
/// @notice {ITalismanRenderer} implementation. Builds the full ERC-721
///         metadata document - name, trait list, SVG image, and a
///         self-contained HTML viewer for `animation_url` - and returns it as
///         a base64-encoded `data:application/json` URI.
...
///      deploying a fresh renderer with the new dependencies and calling
///      `Talismans.setRenderer` - there is no in-place mutability here.
contract TalismanRendererV2 is ITalismanRenderer {
    ...
    TalismanSvgRendererV2 public immutable svgRenderer;
    ...
    constructor(
        ...
        TalismanSvgRendererV2 svgRendererContract,
    ...
    ///      live tokenURI path and {htmlFromTraits}. The page background is set
    ///      to the scene's own black, so the document's first paint already
    ///      matches the rendered artwork.
    function _renderHtml(TalismanGeneratorV2.Talisman memory tal) private view returns (string memory) {
        ...
        string memory html =
            liteRenderer.renderHtml(tal.triangles, camera, tal.materials, settings, light, liteSettings);
        return LibString.replace(html, "html,body{margin:0", "html,body{background:#000;margin:0");
    }
    ...
    function _attributes(TalismanGeneratorV2.Talisman memory tal, bool genesis, uint16 seed)
        private
        view
        returns (string memory)
    {
