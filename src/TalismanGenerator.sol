...
// World-space max radius after fit. Camera frames the mesh in front of this.
// File-scope so callers without a contract instance can still reference it.
int256 constant VIEWPORT_RADIUS = 2e18;

/// @title TalismanGenerator
/// @notice Deterministic brilliant-cut gem generator. From a talisman's packed
///         identity it drives the per-form geometry pipeline in {TalismanForms},
///         fits the mesh to the viewport, then bakes a per-triangle face color
///         from the material record in {TalismanMaterials}. The facet tier sets
///         the polygon count, so rarity reads in the geometry.
...
contract TalismanGenerator {
    ...
    ///      root expected by the existing geometry/color picks. Tagging keeps
    ///      this stream disjoint from any other use of `seed`.
    ...
    function _generateFor(
        ...
        tal.triangles = TalismanForms.triangulateForForm(form, pts, N);
