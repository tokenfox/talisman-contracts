 ...
 * background. Vertices are written to one decimal place, so the image holds its precision at
 * any display size. The gem material carries the chromatic identity; the backdrop is a fixed
 * neutral.
 * @dev Coordinates are carried through the viewport mapping in tenths of a unit and written out
 * as decimals. Culling is unaffected by that scaling: the winding test runs on the projected
 * point, not on the emitted coordinate.
 */
contract TalismanSvgRendererV2 {
    // SVG constants
    ...
    // Draws a triangle as an SVG polygon with material color. Coordinates arrive in tenths.
    ...
    /// @dev Writes a tenths-scaled value as a one-decimal string: 2851 -> "285.1", -37 -> "-3.7".
    ///      The sign is taken before the split because division truncates toward zero, which
    ///      would otherwise put a minus on the fractional digit too.
    function _tenthsToString(int256 v) internal pure returns (string memory) {
        string memory sign = "";
        if (v < 0) {
            sign = "-";
            v = -v;
        ...
        uint256 u = uint256(v);
        return string.concat(sign, LibString.toString(u / 10), ".", LibString.toString(u % 10));
    }
    ...
    /// @dev Viewport mapping, in tenths of an SVG unit. Scaling before the divide keeps a digit
    ///      the whole-unit grid would drop; {_tenthsToString} places the decimal point.
    function _projectOnSvg(Point2D memory p) internal pure returns (int256 x, int256 y) {
        x = 10 * (SVG_WIDTH / 2) + (p.x * 10) / (WAD / (SVG_WIDTH / 2));
        // Remove Y-axis flipping to match Three.js right-handed coordinates
        y = 10 * (SVG_HEIGHT / 2) - (p.y * 10) / (WAD / (SVG_HEIGHT / 2));
    }
